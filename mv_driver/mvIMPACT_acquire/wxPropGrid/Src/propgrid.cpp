...
#include <wxPropGrid/Include/odcombo.h>
...
#define DOUBLE_CLICK_CONVERSION_THRESHOLD        500
...
//once inline function, outlined due to possible bug in GCC 4.5 
wxPGId wxPropertyGrid::AppendIn( wxPGId id, wxPGProperty* property )
{
...
int wxPGProperty::InsertChoice( const wxString& label, int index, int value )
{
    ...
    if ( ci.m_choices )
    {
        ...
        if ( index < 0 )
            index = static_cast<int>(ci.m_choices->GetCount());
...
void wxPGPropertyWithChildren::AddChild2( wxPGProperty* prop, int index, bool correct_mode )
{
    ...
    {
        if ( correct_mode ) prop->m_arrIndex = static_cast<int>(m_children.GetCount());
...
{
    prop->m_arrIndex = static_cast<int>(m_children.GetCount());
    m_children.Add( (void*)prop );
...
{
    size_t i;
    const size_t cnt = GetCount();
    for ( i=starthere;i<cnt;i++)
        Item(i)->m_arrIndex = static_cast<int>(i);
}
...
void wxPGTextCtrlEditor::SetValueToUnspecified( wxWindow* ctrl ) const
{
    ...
    tc->Remove(0,static_cast<long>(tc->GetValue().length()));
}
            ...
            if ( m_combo->GetTextRect().wxPGRectContains(pt) )
            {
                ...
                else if ( evtType == wxEVT_LEFT_UP )
                {
                    if ( m_downReceived || m_timeLastMouseUp == 1 )
                    {
                        ...
                        if ( timeFromLastUp < DOUBLE_CLICK_CONVERSION_THRESHOLD )
                        {
...
public:
    ...
    void SetColour2( const wxColour& colour );
    inline long GetColourAsLong() const { return m_colAsLong; }
    inline wxColour GetColour() const { return wxColour(m_colAsLong & 0xFF, ( m_colAsLong >> 8 ) & 0xFF, ( m_colAsLong >> 16 ) & 0xFF); }
private:
    long    m_colAsLong;
};
...
void wxPropertyGrid::SetPropertyBackgroundColour( wxPGId id, const wxColour& colour )
{
    ...
    if ( colInd < 0 )
    {
        colInd = static_cast<int>(m_arrBgBrushes.GetCount());
...
int wxPropertyGrid::CacheColour( const wxColour& colour )
{
    ...
    // As it is most likely that the previous colour is used, start comparison
    // from the end.
    for ( size_t i=(m_arrFgCols.GetCount()-1); i>0; i-- )
    {
    ...
    if ( colInd < 0 )
    {
        colInd = static_cast<int>(m_arrFgCols.GetCount());
...
{
    wxASSERT( priorthis );
    return FROM_STATE(DoInsert(priorthis->GetParent(), static_cast<int>(priorthis->GetArrIndex()), property));
}
...
bool wxPropertyGrid::EnableProperty( wxPGId id, bool enable )
{
    ...
    if ( enable )
    {
        ...
        if ( p == m_selected )
        {
            ...
            DoSelectProperty((wxPGProperty*)NULL);
        }
    }
    else
    {
...
void wxPropertyGrid::SLAlloc( unsigned int itemcount, const wxChar** items )
{
    ...
    size_t i;
    size_t sl_oldcount = sl.GetCount();
...
wxString& wxPropertyGrid::CreateEscapeSequences( wxString& dst_str, wxString& src_str )
{
    ...
    for ( ; i != src_str.end(); i++ )
    {
        ...
        if ( a >= wxT(' ')
        #if !wxUSE_UNICODE
             || static_cast<wxSChar>(a) < 0 // BAT: Some platforms(ARM) define wxChar as unsigned char which results in a compiler warning if te cast is removed
        #endif
           )
        {
...
wxPGProperty* wxPropertyGrid::DoGetItemAtY( int y )
{
    ...
    size_t index = (unsigned int)((y - vy) / m_lineHeight);
...
bool wxPropertyGridState::EnableCategories( bool enable )
{
    ...
    if ( enable )
    {
        ...
        ITEM_ITERATION_LOOP_BEGIN
            ...
            p->m_arrIndex = static_cast<int>(i);
