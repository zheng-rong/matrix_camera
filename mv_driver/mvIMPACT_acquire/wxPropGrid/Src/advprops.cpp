...
void wxSystemColourPropertyClass::DoSetValue( wxPGVariant value )
{
    ...
    {
        m_index = static_cast<int>(m_choices.GetCount())-1;
    }
}
...
wxMultiChoicePropertyClass::wxMultiChoicePropertyClass(const wxString& label,
                                                       ...
                                                       const wxPGChoices& choices,
                                                       const wxArrayInt& value)
                                                         : wxPGProperty(label,name), m_emptySelectionString(wxEmptyString)
{
...
void wxMultiChoicePropertyClass::SetEmptySelectionString( const wxString& str )
{
    m_emptySelectionString = str;
}
...
void wxMultiChoicePropertyClass::SetValueI( const wxArrayInt& arr )
{
    ...
    {
        m_display = m_emptySelectionString;
    }
}
...
void wxMultiChoicePropertyClass::GenerateValueAsString()
{
    ...
    size_t i;
    size_t itemCount = indices.GetCount();
    ...
    {
        tempStr = m_emptySelectionString;
    }
}
                                          ...
                                          wxEvent& event )
{
    if ( event.GetEventType() == wxEVT_COMMAND_BUTTON_CLICKED )
    {
        ...
        wxMultiChoiceDialog dlg( propgrid,
                                 _("Make a selection:"),
                                 m_label,
                                 static_cast<int>(m_choices.GetCount()),
...
int wxMultiChoicePropertyClass::GetChoiceInfo( wxPGChoiceInfo* choiceinfo )
{
    ...
    {
        if ( m_choices.IsOk() )
            choiceinfo->m_itemCount = static_cast<int>(m_choices.GetCount());
                                                  ...
                                                  int WXUNUSED(argFlags) )
{
	...
	return m_valueDateTime.ParseFormat(text, wxString(wxDefaultDateTimeFormat), wxDefaultDateTime, NULL);
#else
	return ( m_valueDateTime.ParseFormat(text, wxDefaultDateTimeFormat) ) ? true : false;
#endif
}
