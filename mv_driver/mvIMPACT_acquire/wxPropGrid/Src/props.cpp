...
bool wxFloatPropertyClass::SetValueFromString( const wxString& text, int argFlags )
{
    ...
    // ToDouble returns false for strings like '0.45 dB' even if value after this
    // call contains the correct data. This we don't want as our strings might contain
    // unit information. So here we might need to remove the return value check.
    if ( res )
    {
        if ( m_value != value )
        {
...
void wxFlagsPropertyClass::Init()
{
    ...
    size_t prevChildCount = m_children.GetCount();
        ...
        if ( state )
        {
            ...
            if ( selected )
            {
                if ( selected->GetParent() == this )
                    oldSel = static_cast<int>(selected->GetArrIndex());
...
void wxCustomPropertyClass::SetAttribute ( int id, wxVariant& value )
{
    ...
    {
        function_cast<wxPGPaintCallback> fc;
        fc.pI = value.GetVoidPtr();
        m_paintCallback = fc.pO;
        if ( fc.pI )
