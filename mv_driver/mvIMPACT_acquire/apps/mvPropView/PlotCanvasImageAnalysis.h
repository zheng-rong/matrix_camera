...
protected:
	...
	int							GetPercentageToDraw( void ) const { return ( ( 100 - m_DrawStart_percent ) < m_DrawWindow_percent ) ? ( 100 - m_DrawStart_percent ) : m_DrawWindow_percent; }
...
public:
	...
	TDisplayMethod				GetDisplayMethod( void ) const { return m_DisplayMethod; }
	const wxString&				GetConfigName( void ) const { return m_ConfigName; }
	...
	bool						IsAOIFullModeActive( void ) const { return m_boAOIFullMode; }
	void						SetAOIFullMode( bool boActive );
	void						SetAOIColour( const wxColour& );
	const AOI*					GetAOI( void ) const { return m_pAOI; }
	wxRect						GetAOIRect( void ) const { return wxRect(m_AOIx, m_AOIy, m_AOIw, m_AOIh); }
	void						SetProcessBayerParity( bool boActive );
	bool						GetProcessBayerParity( void ) const { return m_boProcessBayerParity; }
	bool						HasPlotSelection( void ) const { return !m_PlotNames.empty(); }
	const std::vector<wxString>& GetAvailablePlotSelections( void ) const { return m_PlotNames; }
	virtual wxString			GetPlotSelection( void ) const { return wxString(wxT("")); }
	virtual void				SetPlotSelection( const wxString& /*plotName*/ ) {}
	bool						HasFeature( TPlotFeature feature ) const { return m_plotFeatures.find( feature ) != m_plotFeatures.end(); }
	void						SetHistoryDepth( unsigned int historyDepth );
	unsigned int				GetHistoryDepth( void ) const { return m_HistoryDepth; }
	...
	ImageCanvas*				GetImageCanvas( void ) const { return m_pImageCanvas; }
};
