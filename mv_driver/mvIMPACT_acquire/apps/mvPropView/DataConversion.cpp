...
//------------------------------------------------------------------------------
{
	switch( c )
	{
	case 'i': return string("%d");
	case 'I': return string(MY_FMT_I64);
	case 'f': return string("%f");
	case 's': return string("%s");
	case 'p': return string("%p");
	}
	return string("");
}
...
//------------------------------------------------------------------------------
{
	switch( c )
	{
	case 'v': return string("void");
	case 'p': return string("void*");
	case 'i': return string("int");
	case 'I': return string("int64");
	case 'f': return string("float");
	case 's': return string("char*");
	}
	return string("");
}
...
GlobalDataStorage::GlobalDataStorage() : componentVisibility_(cvBeginner), boComponentVisibilitySupported_(false),
  LIST_BACKGROUND_COLOUR_(230, 230, 230), IS_DEFAULT_VALUE_COLOUR_(5, 165, 5), INVISIBLE_GURU_FEATURE_COLOUR_(255, 192, 64)
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
{
	...
	case pgcIsDefaultValue:
		return IS_DEFAULT_VALUE_COLOUR_;
	default:
		break;
	}
