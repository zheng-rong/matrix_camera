...
protected:
	...
	virtual bool			FeatureChangedCallbacksSupported( void ) const { return false; }
	virtual bool			FeatureHasChangedCallback( mvIMPACT::acquire::Component ) const { return false; }
			wxPropertyGrid*	GetPropertyGrid( void ) { return m_pPGSelected; }
	virtual EDisplayFlags	GetDisplayFlags( void ) const { return dfNone; }
	...
	virtual bool			ShowPropGridMethodExecutionErrors( void ) const { return true; }
...
public:
	...
	virtual				   ~PropGridFrameBase();
			...
			void			StartPropertyGridUpdateTimer( void ) { StartPropertyGridUpdateTimer( DEFAULT_PROP_GRID_UPDATE_PERIOD ); }
...
public:
	explicit PropGridUpdateTimerSuspendScope( PropGridFrameBase* p ) : p_(p) { p_->StopPropertyGridUpdateTimer(); }
	~PropGridUpdateTimerSuspendScope() { p_->StartPropertyGridUpdateTimer(); }
};
