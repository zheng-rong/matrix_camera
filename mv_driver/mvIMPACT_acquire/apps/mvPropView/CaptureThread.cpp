...
public:
	TimerLite() : m_endClocks(0) {;}
	TimerLite( unsigned long nextDelay_ms )
	{
...
//-----------------------------------------------------------------------------
{
	if( !m_currentPlotInfoPath.empty() )
	{
		...
		switch( p.type() )
		{
		case ctPropInt:   pReqInfo->plotValue.value.intRep    = PropertyI(p).read(); break;
		case ctPropInt64: pReqInfo->plotValue.value.int64Rep  = PropertyI64(p).read(); break;
		case ctPropFloat: pReqInfo->plotValue.value.doubleRep = PropertyF(p).read(); break;
		default: break;
		}
	}
...
//-----------------------------------------------------------------------------
int CaptureThread::InternalSetLiveMode( bool boOn, bool boInformUserOnRecordStart /* = true */ )
//-----------------------------------------------------------------------------
{
	if( boOn )
	{
		...
		{
			m_boLive = boOn;
			m_pFuncInterface->acquisitionStart();
		}
...
//-----------------------------------------------------------------------------
int CaptureThread::InternalSetRecordMode( bool boOn, bool boUnlockCurrentRequest )
//-----------------------------------------------------------------------------
{
	if( boOn )
	{
...
//-----------------------------------------------------------------------------
int CaptureThread::SetLiveMode( bool boOn, bool boInformUserOnRecordStart /* = true */ )
//-----------------------------------------------------------------------------
{
	wxCriticalSectionLocker locker(m_critSect);
...
//-----------------------------------------------------------------------------
int CaptureThread::SetRecordMode( bool boOn )
//-----------------------------------------------------------------------------
{
	wxCriticalSectionLocker locker(m_critSect);
