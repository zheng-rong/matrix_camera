...
{
	clock_t m_endClocks;
	clock_t Delay2Clocks( unsigned long nextDelay_ms ) const { return ( nextDelay_ms * CLOCKS_PER_SEC ) / 1000; }
public:
	explicit TimerLite() : m_endClocks(0) {}
	explicit TimerLite( unsigned long nextDelay_ms ) { Restart( nextDelay_ms ); }
	void Restart( unsigned long nextDelay_ms ) { m_endClocks = clock() + Delay2Clocks( nextDelay_ms ); }
