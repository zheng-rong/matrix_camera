...
public:
	...
	ImageCanvas* GetDisplayArea( void ) { return m_pDisplayArea; }
...
: PropGridFrameBase(widMainFrame, title, pos, size), m_boCloseDeviceInProgress(false), m_boFindFeatureMatchCaseActive(false),
   m_boDisplayWindowMaximized(false), m_boShutdownInProgress(false),
   ...
   m_defaultDeviceInterfaceLayout(dilGenICam), m_HardDiscRecordingParameters()
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
{
...

	{
		...
		pConfig->Write( wxT("/MainFrame/Settings/defaultDeviceInterfaceLayout"), int(m_defaultDeviceInterfaceLayout) );
...
//-----------------------------------------------------------------------------
{
	...
	case miAction_DefaultDeviceInterface_DeviceSpecific:
		m_defaultDeviceInterfaceLayout = dilDeviceSpecific;
		break;
	case miAction_DefaultDeviceInterface_Generic:
		m_defaultDeviceInterfaceLayout = dilGeneric;
		break;
	case miAction_DefaultDeviceInterface_GenICam:
		m_defaultDeviceInterfaceLayout = dilGenICam;
		break;
	}
...
//-----------------------------------------------------------------------------
{
	...
	{
		m_ViewMode = DevicePropertyHandler::vmStandard; // for backwards compatibility
	}
	m_defaultDeviceInterfaceLayout = static_cast<TDeviceInterfaceLayout>(pConfig->Read( wxT("/MainFrame/Settings/defaultDeviceInterfaceLayout"), dilGenICam ));
	m_pAcquisitionModeCombo->SetValue( m_ContinuousStr );
...
//-----------------------------------------------------------------------------
{
	...
	m_pMICapture_Acquire->Enable( boDevOpen && boDevPresent );
...
//-----------------------------------------------------------------------------
{
	...
	if( pDev )
	{
		...
		try
		{
			...
			{
				if( pDev->interfaceLayout.isValid() && ( pDev->interfaceLayout.read() == dilGeneric ) )
				{
					...
					{
						WriteLogMessage( wxString::Format( wxT("Opening of device '%s' in 'Generic' interface layout canceled by user.\n"), ConvertedString(pDev->serial.read()).c_str() ) );
						return;
					}
				}
	...
	for( unsigned int i=0; i<m_DeviceCount; i++ )
	{
		try
		{
			if( devMgr[i]->interfaceLayout.isValid() &&
				...
				( devMgr[i]->interfaceLayout.read() != m_defaultDeviceInterfaceLayout ) )
			{
				std::vector<std::pair<std::string, TDeviceInterfaceLayout> > dict;
				devMgr[i]->interfaceLayout.getTranslationDict( dict );
				std::vector<std::pair<std::string, TDeviceInterfaceLayout> >::const_iterator it = find_if( dict.begin(), dict.end(), SecondMatches<const string, TDeviceInterfaceLayout>(make_pair( "", m_defaultDeviceInterfaceLayout )) );
				if( it != dict.end() )
				{
					devMgr[i]->interfaceLayout.write( m_defaultDeviceInterfaceLayout );
					...
					else
					{
						...
						if( !file.fail() )
						{
							...
							if( file > 0 )
							{
								...
								wxProgressDialog dlg(wxString::Format( wxT("Downloading file '%s' from device '%s' into RAM"), fileNameDevice.c_str(), ConvertedString(pDev->serial.read()).c_str() ),
								...
								while( bytesRead < fileSize )
								{
									...
									dlg.Update( bytesRead, wxString::Format( wxT("%s bytes read"), ConvertedString(progress.str()).c_str() ) );
								}
								if( file.good() )
								{
