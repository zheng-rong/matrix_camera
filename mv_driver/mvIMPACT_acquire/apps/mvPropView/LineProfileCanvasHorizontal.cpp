...
LineProfileCanvasHorizontal::LineProfileCanvasHorizontal( wxWindow* parent, wxWindowID id /* = -1 */, const wxPoint& pos /* = wxDefaultPosition */,
	const wxSize& size /* = wxDefaultSize */, long style /* = wxSUNKEN_BORDER */, const wxString& name /* = "Horizontal Line Profile" */, bool boActive /* = false */ )
...
//-----------------------------------------------------------------------------
{
	...
	case ibpfYUV422Packed:
		ProcessYUVData<unsigned char>( pIB );
		break;
	case ibpfYUV422_10Packed:
		ProcessYUVData<unsigned short>( pIB );
		break;
	case ibpfYUV422_UYVYPacked:
		ProcessUYVData<unsigned char>( pIB );
		break;
	case ibpfYUV422_UYVY_10Packed:
		ProcessUYVData<unsigned short>( pIB );
		break;
	...
	case ibpfYUV422Planar:
		for( int line=0; line<m_AOIh; line++ )
		{
			unsigned char* py = static_cast<unsigned char*>(pIB->vpData) + pIB->pChannels[pRed].iChannelOffset + ( ( m_AOIy + line ) * pIB->pChannels[pY].iLinePitch ) + m_AOIx;
			unsigned char* pu = static_cast<unsigned char*>(pIB->vpData) + pIB->pChannels[pGreen].iChannelOffset + ( ( m_AOIy + line ) * pIB->pChannels[pU].iLinePitch ) + m_AOIx;
			unsigned char* pv = static_cast<unsigned char*>(pIB->vpData) + pIB->pChannels[pBlue].iChannelOffset + ( ( m_AOIy + line ) * pIB->pChannels[pV].iLinePitch ) + m_AOIx;
			for( int pixel=0; pixel<m_AOIw; pixel++ )
			{
...
//-----------------------------------------------------------------------------
template<typename _Ty>
void LineProfileCanvasHorizontal::ProcessYUVData( const ImageBuffer* pIB )
//-----------------------------------------------------------------------------
{
	if( pIB->iWidth > 1 )
	{
...
//-----------------------------------------------------------------------------
template<typename _Ty>
void LineProfileCanvasHorizontal::ProcessUYVData( const ImageBuffer* pIB )
//-----------------------------------------------------------------------------
{
	if( pIB->iWidth > 1 )
	{
