...
public:
	...
	explicit				WizardLUTCanvas( wxWindow* parent, wxWindowID id = -1, const wxPoint& pos = wxDefaultPosition,
						   ...
						   ~WizardLUTCanvas();
	int						GetPercentageToDraw( void ) const { return 100; }
...
//-----------------------------------------------------------------------------
{
	...
	void WriteErrorMessage( const wxString& msg ) { WriteLogMessage( msg, errorStyle_ ); }
	void WriteLogMessage( const wxString& msg, const wxTextAttr& style = wxTextAttr(*wxBLACK) ) { WriteToTextCtrl( pLogWindow_, msg, style ); }
	void WriteLUTToDevice( const wxString& LUTSelectorValue, std::vector<int>& lut );
protected:
	virtual void OnBtnApply( wxCommandEvent& );
	virtual void OnBtnCancel( wxCommandEvent& ) { Hide(); }
	void OnBtnCopyTo( wxCommandEvent& );
	void OnBtnDisableAll( wxCommandEvent& ) { ConfigureAllLUTs( false ); }
	void OnBtnEnableAll( wxCommandEvent& )  { ConfigureAllLUTs( true ); }
	...
	virtual void OnBtnOk( wxCommandEvent& ) { Hide(); }
...
//-----------------------------------------------------------------------------
{
	...
	void OnGammaChanged( wxSpinEvent& )                      { data_.gamma_ = GetGamma();                             InformParentAboutChanges(); }
	void OnGammaTextChanged( wxCommandEvent& )               { data_.gamma_ = GetGamma();                             InformParentAboutChanges(); }
	void OnGammaAlphaChanged( wxSpinEvent& )                 { data_.gammaAlpha_ = GetGammaAlpha();                   InformParentAboutChanges(); }
	void OnGammaAlphaTextChanged( wxCommandEvent& )          { data_.gammaAlpha_ = GetGammaAlpha();                   InformParentAboutChanges(); }
	void OnGammaStartThresholdChanged( wxSpinEvent& )        { data_.gammaStartThreshold_ = GetGammaStartThreshold(); InformParentAboutChanges(); }
	void OnGammaStartThresholdTextChanged( wxCommandEvent& ) { data_.gammaStartThreshold_ = GetGammaStartThreshold(); InformParentAboutChanges(); }
	void SetupControls( void );
	void InformParentAboutChanges( void ) const { wxCommandEvent e(GammaParameterChangeEvent, GetParent()->GetId()); ::wxPostEvent( GetParent()->GetEventHandler(), e ); }
public:
	explicit WizardLUTGammaDlg( wxWindow* pParent, const wxString& title, const int maxLUTIndex, GammaData& data );
	double GetGamma( void ) const { return pSCGamma_->GetValue(); }
	double GetGammaAlpha( void ) const { return pSCGammaAlpha_->GetValue(); }
	wxString GetGammaMode( void ) const { return pCBGammaMode_->GetValue(); }
	int GetGammaStartThreshold( void ) const { return pSCGammaStartThreshold_->GetValue(); }
};
