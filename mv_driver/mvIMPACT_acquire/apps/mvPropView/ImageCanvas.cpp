...
#endif
		wxCROSSDIAG_HATCH), format(ibpfRaw), currentShiftValue(0), appliedShiftValue(0) {}
...
//-----------------------------------------------------------------------------
{
	...
	//-----------------------------------------------------------------------------
	template<typename _Ty>
	void ConvertPackedYUVToRGB( const ImageBuffer* pIB, PixelData::Iterator& p, PixelData& data, const int shift = 0 )
	//-----------------------------------------------------------------------------
	{
	...
	//-----------------------------------------------------------------------------
	template<typename _Ty>
	void ConvertPackedYUVToRGB( const ImageBuffer* pIB, unsigned char* pDst, const int shift = 0 )
	//-----------------------------------------------------------------------------
	{
		for( int line=0; line<pIB->iHeight; ++line )
		{
...
//-----------------------------------------------------------------------------
{
	...
	if( !m_pImpl->currentImage.Ok() || ( m_pImpl->currentImage.GetWidth() != pIB->iWidth ) ||
	...
	case ibpfYUV422_UYVYPacked:
		ConvertPackedYUVToRGB<unsigned char>( pIB, p, data );
		break;
	case ibpfYUV422_10Packed:
	case ibpfYUV422_UYVY_10Packed:
		ConvertPackedYUVToRGB<unsigned short>( pIB, p, data, m_pImpl->GetShift( 2 ) );
		break;
	...
	case ibpfYUV422Packed:
	case ibpfYUV422_UYVYPacked:
		ConvertPackedYUVToRGB<unsigned char>( pIB, pDst );
		break;
	case ibpfYUV422_10Packed:
	case ibpfYUV422_UYVY_10Packed:
		ConvertPackedYUVToRGB<unsigned short>( pIB, pDst, m_pImpl->GetShift( 2 ) );
		break;
	case ibpfYUV444_UYVPacked:
		{
			const int order[3] = { 1, 0, 2 };
