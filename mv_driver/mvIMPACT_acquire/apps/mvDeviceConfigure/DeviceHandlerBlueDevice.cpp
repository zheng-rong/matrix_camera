...
//------------------------------------------------------------------------------
class ProgressBarEraseFlashThread : public wxThread
//------------------------------------------------------------------------------
{
    mvIMPACT::acquire::GenICam::FileAccessControl    fac_;
    int*                                             pFileOperationExecuteResult_;
protected:
    void* Entry()
    {
        ...
        *pFileOperationExecuteResult_ = fac_.fileOperationExecute.call();
        return 0;
    }
public:
    explicit ProgressBarEraseFlashThread( mvIMPACT::acquire::GenICam::FileAccessControl& fac, int* pFileOperationExecuteResult ) : wxThread( wxTHREAD_JOINABLE ), fac_( fac ), pFileOperationExecuteResult_( pFileOperationExecuteResult ) {}
};

//------------------------------------------------------------------------------
class ProgressBarWaitForCloseThread : public wxThread
//------------------------------------------------------------------------------
{
...
public:
    explicit ProgressBarWaitForCloseThread(  mvIMPACT::acquire::GenICam::ODevFileStream* pUploadFile ) : wxThread( wxTHREAD_JOINABLE ), pUploadFile_( pUploadFile ) {}
};
...
//-----------------------------------------------------------------------------
{
    static const int MAX_UPDATE_TIME_MILLISECONDS = 180000; // 3 minutes
    ...
    TUpdateResult result = urOperationSuccessful;
    {
        ...
        if( uf.transferBuffer_.isValid() )
        {
            ...
            wxProgressDialog dlg( wxT( "Applying Firmware Update" ),
                                  wxT( "This may take several minutes..." ),
            ...
            if( pParent_ )
            {
                pParent_->WriteLogMessage( wxString::Format( wxT( "Result of updating device %s(%s): %s.\n" ), ConvertedString( serial ).c_str(), ConvertedString( pDev_->product.read() ).c_str(), ConvertedString( uf.lastResult_.read() ).c_str() ) );
            }
        }
        else
        {
            if( pParent_ )
            {
                pParent_->WriteLogMessage( wxString::Format( wxT( "Failed to update device %s(%s)(Feature 'TransferBuffer' is not implemented, a driver update is needed).\n" ), ConvertedString( serial ).c_str(), ConvertedString( pDev_->product.read() ).c_str() ) );
            }
...
//-----------------------------------------------------------------------------
{
    ...
    ProgressBarEraseFlashThread updateThread( fac, &fileOperationExecuteResult );
    updateThread.Create();
    updateThread.Run();
    while( updateThread.IsRunning() )
    {
        ...
        if( !uploadFile.fail() )
        {
            ...
            while( bytesWritten < bytesToWriteTotal )
            {
                ...
                progress << std::setw( 8 ) << std::setfill( '0' ) << bytesWritten << '/'
                         << std::setw( 8 ) << std::setfill( '0' ) << bytesToWriteTotal;
                dlgFWUpdate.Update( bytesWritten, wxString::Format( wxT( "Applying Firmware Update(%s bytes written)" ), ConvertedString( progress.str() ).c_str() ) );
            }
            if( !uploadFile.good() )
            {
    ...
    static const int s_MAX_SLEEP_TIME_S = 60;
    static const int s_NAP_TIME_S = 3;
    int timeSlept_s = 0;
    DeviceManager devMgr;
    wxStopWatch stopWatch;
    // Wait up to one minute for the device to reboot. If it doesn't re-appear
    ...
    while( timeSlept_s < s_MAX_SLEEP_TIME_S )
    {
        wxSleep( s_NAP_TIME_S );
        timeSlept_s += s_NAP_TIME_S;
        devMgr.updateDeviceList(); // this is needed in order to invalidate cached data that might no longer be valid(e.g. the URLs for the description files)
...
//-----------------------------------------------------------------------------
bool DeviceHandlerBlueDevice::IsBlueFOX3( void ) const
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
int DeviceHandlerBlueDevice::UpdateCOUGAR_XAndFOX3Device( bool boSilentMode, bool boPersistentUserSets )
//-----------------------------------------------------------------------------
{
    ConvertedString serial( pDev_->serial.read() );
    wxString firmwareFileAndPath;
    ...
    GetLatestFirmwareVersionCOUGAR_XAndFOX3Device( latestVersion );
    ...
    if( boSilentMode && ( VersionFromString( ConvertedString( pDev_->firmwareVersion.readS() ) ) == fileEntries[0].version_ ) )
    {
...
//-----------------------------------------------------------------------------
{
    ...
    if( product_ == pgBlueCOUGAR_S )
    {
        return UpdateCOUGAR_SDevice( boSilentMode );
             ...
             ( product_ == pgBlueFOX3 ) )
    {
        return UpdateCOUGAR_XAndFOX3Device( boSilentMode, boPersistentUserSets );
    }
    else
    {
