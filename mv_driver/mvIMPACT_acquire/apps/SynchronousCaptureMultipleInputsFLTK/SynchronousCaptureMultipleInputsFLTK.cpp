...
static bool g_boTerminated = false;
	...
	public:
		...
		void NewRequest( const Request* pRequest) { pRequest_ = pRequest; }
		void setOverlayString( const string& msg ) { msg_ = msg; redraw_overlay(); }
	};
...
//-----------------------------------------------------------------------------
{
	...
	// prefill the capture queue. There can be more then 1 queue for some devices, but for this sample
